...
(function ($, fluid) {
    ...
    var serializeKeyValuePair = function (key, value) {
        return key + "=" + encodeURIComponent(value);
    };
    
    /**
     * Converts an object's key/value pairs into a URL-friendly query string.
     * 
     * @param {Object} obj the object to convert into a query string
     */
    fluid.toQueryString = function (obj) {
        var queries = [];
        ...
        $.each(obj, function (name, value) {
            ...
            if (value && typeof value !== "string" && typeof value.length === "number") {
                $.each(value, function (idx, arrayVal) {
                    queries.push(serializeKeyValuePair(name, arrayVal)); 
                });
            } else {
                queries.push(serializeKeyValuePair(name, value));
            }
        });
