...
(function ($, fluid) {
    ...
    var prepareUpstreamOptions = function (flashUrl, flashContainer) {
        //  fluid.allocateSimpleId(flashContainer.children().eq(0)),
        return {
            flashURL: flashUrl || undefined,
    ...
    var setupForFlash9 = function (that) {
        that.returnedOptions = prepareUpstreamOptions(that.options.flash9Url, createFlash9MovieContainer(that));
    };
    ...
    var setupForFlash10 = function (that, uploadManager, uploaderContainer, browseButton) {
        var o = that.options,
            flashContainer = createFlash10MovieContainer(that, uploaderContainer);
        
        fluid.tabindex(browseButton, -1);
        that.isTransparent = o.flashButtonAlwaysVisible ? false : (!$.browser.msie || o.transparentEvenInIE);
        that.returnedOptions = $.extend({}, prepareUpstreamOptions(o.flash10URL, flashContainer), {
            ...
            listeners: {
                onUploadStart: function () {
                    uploadManager.swfUploader.setButtonDisabled(true);
                },
                afterUploadComplete: function () {
                    uploadManager.swfUploader.setButtonDisabled(false);
                }
            }  
        });
    };
    ...
    /**
     * SWFUploadView is designed to setup the DOM correctly for SWFUpload and configure
     ...
     * @param {UploadManager} the parent UploadManager
     * @param {options} options configuration options for the decorator
     */
    fluid.uploader.swfUploadView = function (uploader, uploadManager, options) {
        var that = fluid.initLittleComponent("fluid.uploader.swfUploadView", options);
               
        that.flashVersion = swfobject.getFlashPlayerVersion().major;
        if (that.flashVersion === 9) {
            setupForFlash9(that);
        } else {
            setupForFlash10(that, uploadManager, uploader.container, uploader.locate("browseButton"));
        }
        
        return that;
    };
    
    fluid.defaults("fluid.uploader.swfUploadView", {
    ...
    var bindEvents = function (that) {
        ...
        that.events.onFileProgress.addListener(function (file, currentBytes, totalBytes) {
            that.queueManager.updateBatchStatus(currentBytes);
            fileStatusUpdater(file);
        });
        
        that.events.onFileError.addListener(function (file, error) {
            that.queueManager.error(file, error);
            fileStatusUpdater(file);
        });
        
        that.events.onFileSuccess.addListener(function (file) {
            that.queueManager.success(file);
            fileStatusUpdater(file);
        });
    ...
    var setupSwfUploadManager = function (that, events) {
        ...
        // Instantiate the SWFUploadView to setup the DOM for Flash.
        that.view = fluid.initSubcomponent(that, 
                                           "view", 
                                           [that.uploader, that, fluid.COMPONENT_OPTIONS]);
     ...
     */
    fluid.uploader.swfUploadManager = function (events, options, uploader) {
        var that = {
            uploader: uploader
        };
        
        // This needs to be refactored!
        fluid.mergeComponentOptions(that, "fluid.uploader.swfUploadManager", options);
    ...
    fluid.defaults("fluid.uploader.swfUploadManager", {
        view: {
            type: "fluid.uploader.swfUploadView"
        },
